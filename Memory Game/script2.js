let flipedCardArray = []; // permet de sauvgarder les images retounées
...
class Card {
	...
	createCard() {
		...
		divCard.addEventListener("click", () => {
			const isFlipped = divCard.classList.contains("fliped");
			if (isFlipped) {
				divCard.style.transform = "rotateY(0deg)";
				divInner.style.transform = "rotateY(0deg)";
				divBack.style.transform = "rotateY(0deg)";
				divCard.classList.remove("fliped");

				return;
			}
			divCard.classList.add("fliped");
			flipedCardArray.push(this.image);
			divCard.style.transform = "rotateY(180deg)";
			divInner.style.transform = "rotateY(180deg)";
			divBack.style.transform = "rotateY(180deg)";
			// check if it same
			if (flipedCardArray.length === 2) {
				const first_card = flipedCardArray.at(0);
				const second_card = flipedCardArray.at(1);
